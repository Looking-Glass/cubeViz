...
static VS_FIXEDFILEINFO driver_version[WDI_NB_DRIVERS-2] = { {0}, {0}, {0} };
static const char* driver_name[WDI_NB_DRIVERS-2] = {"winusbcoinstaller2.dll", "libusb0.sys", "libusbK.sys"};
static const char* inf_template[WDI_NB_DRIVERS-2] = {"winusb.inf.in", "libusb0.inf.in", "libusbk.inf.in"};
static const char* cat_template[WDI_NB_DRIVERS-2] = {"winusb.cat.in", "libusb0.cat.in", "libusbk.cat.in"};
static const char* ms_compat_id[WDI_NB_DRIVERS-2] = {"MS_COMP_WINUSB", "MS_COMP_LIBUSB0", "MS_COMP_LIBUSBK"};
...
// extract the embedded binary resources,subdir!=NULL时,仅将子目录中的释放到path
static int extract_binaries(const char* path,const char *subdir)
{
	FILE *fd;
	char filename[MAX_PATH];
	int i, r;
//	char *szDriverDir[2] = { "usb_driverXP", "usb_driverVISTA"};
//	int ki = 0,skip=1;
// 	GET_WINDOWS_VERSION;
...
// 	}
	...
	for (i=0; i<nb_resources; i++) {
		...
		if (resource[i].subdir[0] == 0) //没有子目录,则跳过释放
			continue;
		//若当前是VISTA以上的，就跳过XP的，否则相反
// 		if (strncmp(resource[i].subdir, szDriverDir[skip], strlen(szDriverDir[skip])) == 0)
// 			continue;
		pos=0;
		if(subdir)
		{
			if (strncmp(resource[i].subdir, subdir, ln) == 0)	//有相同目录
			{
//				wchar_t	ss[3] = { 0, 0, 0 }; ss[0] = 0x30 + i + 1;
//				MessageBox(NULL, ss, NULL, MB_OK);
		...
		safe_strcat(filename, MAX_PATH, "\\");
		safe_strcat(filename, MAX_PATH, resource[i].subdir+pos);
								  ...
								  const char* inf_name, struct wdi_options_prepare_driver* options)
{
	...
	const char* driver_display_name[WDI_NB_DRIVERS] = { "WinUSB", "libusb0.sys", "libusbK.sys", "user driver", "user driver2" };
#endif
	...
	char *subdir[4] = { "usb_driverXP", "usb_driverVISTA", "usb_driverCom", "usb_driverDfu"};
	int ki=0;
	int wdfver;

	MUTEX_START;
	wdfver=wdi_get_wdf_version();
	...
	if ( (driver_type < 0) || (driver_type >= WDI_NB_DRIVERS) ) {
		wdi_err("unknown type");
		MUTEX_RETURN(WDI_ERROR_INVALID_PARAM);
	}

	if (driver_type!=WDI_USER&&driver_type!=WDI_USER2) //用于USB2COM安装,bywbj 20151204
	...
	if (driver_type == WDI_USER) {
		...
		MUTEX_RETURN(extract_binaries(path,subdir[ki]));
	}
	if (driver_type == WDI_USER2) {
		ki=3;
		wdi_info("custom driver - extracting binaries only (no inf/cat creation)");
		MUTEX_RETURN(extract_binaries(path,subdir[ki]));
	}
...
//	MessageBox(NULL, "2", NULL, MB_OK);
	...
	r = extract_binaries(path,subdir[ki]);
	if (r != WDI_SUCCESS) {
		MUTEX_RETURN(r);
	}
	...
	static_sprintf(inf_entities[WDF_VERSION].replace, "%05d", wdfver);
	static_sprintf(inf_entities[KMDF_VERSION].replace, "%d.%d", wdfver/1000, wdfver%1000);
